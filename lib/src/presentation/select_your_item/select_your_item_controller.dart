...
class SelectYourItemController extends GetxController {
  ...
  void setQuantity(int laundryItemId, int serviceId, bool isIncrement,
    ...
    print(
        'setQuantity called with laundryItemId: $laundryItemId, serviceId: $serviceId');
          ...
          for (int j = 0; j < cartLisst[i].items!.length; j++) {
            if (cartLisst[i].items![j].laundryItemId == laundryItemId) {
              if (isIncrement) {
                ...
                if((cartLisst[i].items![j].quantity ?? 0) > 1){
                  cartLisst[i].items![j].quantity = (cartLisst[i].items![j].quantity ?? 0) - 1;
                }
                else {
                  cartLisst[i].items!.removeAt(j);
                }
              }
              break;
            }
  ...
  void addtocart(
      int serviceId, int laundryItemId, int quantity, double price, String name,
      {LaundryCartAddonBody? selectedAddon}) {
    LaundryCartBody? cartitem;
    for (int i = 0; i < cartLisst.length; i++) {
        print('cartLisst[i].servicesId: ${cartLisst[i].servicesId}');
      if (cartLisst[i].servicesId == serviceId) {
    ...
    if (cartitem != null) {
      ...
      cartLisst.add(LaundryCartBody(servicesId: serviceId, items: [
        LaundryItemBody(
            laundryItemId: laundryItemId,
            quantity: quantity,
            price: price,
  ...
  Future<void> saveCartToStorage() async {
    List<Map<String, dynamic>> cartjson = cartLisst
        .map((cartitem) => {
